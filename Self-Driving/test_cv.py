...
class MyCam(object):
    ...
    def run(self):
        ...
        vertices = np.array([[(0, self.height),
                            (self.width/2-100, self.height/3),
                            (self.width/2+100, self.height/3),
        ...
        temp = np.zeros((self.height, self.width, 3),
                        dtype=np.uint8)
        self.degree = tilting_restruction(line_arr, temp, 160, 95)
...
def get_fitline(img, f_lines):  # 대표선 구하기
    ...
    if len(lines.shape) > 1:
        ...
        if not np.isnan(vx):
            x1 = int(((rows-1)-y)/vy*vx + x)
            y1 = rows-1
            x2 = int(((rows/2+50)-y)/vy*vx + x)
            y2 = int(rows/2+50)
            result = [x1, y1, x2, y2]
            return result
    ...
    while(cam.cap.isOpened()):
        try:
            ...
            if(cv2.waitKey(30) & 0xFF == 27):
                break
            if(send_image(sock, cam.result) is False):
                break
            print(cam.degree)
